            ...
            <div key={step.step} className="flex items-center gap-2 shrink-0">
              ...
              <button
                ...
                className={`flex flex-col items-start gap-1 px-3 py-2 rounded-lg border text-left transition-all min-w-[150px] ${
                  isSelected
                    ? 'border-white/30 bg-gray-800 ring-1 ring-white/10'
                    : isFailed
              ...
              >
                ...
                <span
                  className={`text-xs font-mono truncate max-w-[180px] ${
                    isFailed ? 'text-red-300' : 'text-gray-500'
                  }`}
                >
                  {step.action}
                </span>
                {hasWarning && (
                  <div className="flex items-center gap-1 mt-0.5">
                    ...
                    <span className="text-[10px] text-yellow-400 truncate max-w-[140px]">
                      {step.warning}
                    </span>
                  </div>
                )}
      ...
      {selectedStep && (
        <div className="mt-3 p-3 rounded-lg bg-gray-800/50 border border-gray-700/50">
          <div className="grid grid-cols-4 gap-3 text-xs">
            <div>
              <span className="text-gray-500 font-mono block">Step</span>
              <span className="text-gray-200 font-mono">{selectedStep.step}</span>
            </div>
            <div>
              <span className="text-gray-500 font-mono block">Node</span>
              <span className="text-gray-200 font-mono">{selectedStep.node}</span>
            </div>
            <div>
              <span className="text-gray-500 font-mono block">Action</span>
              <span className="text-gray-200 font-mono">{selectedStep.action}</span>
            </div>
            <div>
              <span className="text-gray-500 font-mono block">Memory</span>
              <span
                className={`font-mono ${
